...
@section('pageTitle', Auth::user()->fname."'s Properties")

@section('meta_tags')
	<meta
		name="keywords"
		content="{{ Auth::user()->fname.'\'s Properties' .' ,'}} @lang('messages.meta_keyword_myads')"> 
	<meta
		name="description"
		content="{{ Auth::user()->fname.'\'s Properties' .' |'}} @lang('messages.meta_desc_myads')"> 
@endsection
			...
			<div class="row">
				...
				<form action="{{ route('myads') }}" method="post" id="sidebarForm">
						...
						<div id="listing-header" class="clearfix" style="margin-top: 55px;">
							...
							<div class="form-control-small">
								...
								<select id="cmb_sort_by" name="sortby" data-placeholder="Sort">
									<option 
										{{ ($sortBy == 'date') ? ' selected="selected"':''}}
										value="date">
											@lang('messages.lbl_sort_by_date')
									</option>
									<option
										{{ ($sortBy == 'size') ? ' selected="selected"':''}}
										value="size">
											@lang('messages.lbl_sort_by_area')
									</option>
									<option
										{{ ($sortBy == 'price') ? ' selected="selected"':''}}
										value="price">
											@lang('messages.lbl_sort_by_price')
									</option>
								</select>
								{{-- ./cmb_sort_by --}}
							</div>
							...
							<div class="sort">
								<ul>
									<li
										...
										onclick="javascript:changeSortOrder('desc');">
											<i
												data-toggle="tooltip"
												data-placement="top"
												title="@lang('messages.lbl_sort_descending')"
												class="fa fa-chevron-down"></i>
									</li>
									<li
										class="{{ ($sortOrder == 'asc') ? 'active':'' }}"
										onclick="javascript:changeSortOrder('asc');">
											<i
												data-toggle="tooltip"
												data-placement="top"
												title="@lang('messages.lbl_sort_ascending')"
									...
									@foreach($result as $proData)
										...
										<div class="item col-md-4">
											...
											<div class="price {{ ($proData->is_available == '0')?'red':'' }}">
												...
												@endif
												<span>
													@if(!empty($proData->price_usd) && $proData->price_usd != 'NaN')
														{{ number_format($proData->price_usd/1000,3) }} kr {{ ($proData->action == 'rent') ? '/md':''}}
													@elseif(!empty($proData->price_dk) && $proData->price_dk != 'NaN')
														{{ number_format($proData->price_dk/1000,3) }} kr {{ ($proData->action == 'rent') ? '/md':''}}
													@elseif(!empty($proData->price) && $proData->price != 'NaN')
														{{ number_format($proData->price/1000,3) }} kr {{ ($proData->action == 'rent') ? '/md':''}}
													@else
														{{ number_format(0/1000,3) }} kr {{ ($proData->action == 'rent') ? '/md':''}}
													@endif
												</span>
											</div>
											{{-- ./price --}}
											...
											<div class="info">
												<h3>
													...
													<small>
														{{ ($proData->city_name!='') ? $proData->city_name:'' }}
													</small>
												</h3>
												<p>
													@php
					...
					<div class="sidebar gray col-sm-4">
						<h2 class="section-title">@lang('messages.psearchtitle')</h2>
